...
A checklist of everything `dod.md` requires, tracking what's implemented
...
verification detail per step), see `build-log.md`.
...
- [x] A library produced/modified by `lazypax` stays fully usable from the
      ...
      (`build-log.md` step 13) exercised every write path
...
`dod.md §4` for why these are architectural boundaries rather than
...
- [x] Full loop runnable through `lazypax` alone, no `pax` CLI, no hand-editing
      ...
      and as one continuous chain (`build-log.md` step 13's capstone
      ...
      (see `build-log.md`'s standing lessons), which search→add and
...
- [x] Resulting `research/flake.nix` + `research/papers.nix` byte-for-byte
      ...
      to `pax`'s own embedded template (`build-log.md` step 13's
...
- [ ] Library remains independently reproducible: `git clone` + `nix build`
      ...
      `status.md`), but that's an inference from the shared code path,
...
- [ ] No panic/unhandled `Result`/stuck screen through the happy path or these
  ...
  - Fetching/opening a paper with no PDF source: solidly covered —
    ...
    exercised live (`build-log.md` step 13's diagnostic hit a real
...
**All 13 build-order steps are done** (`build-log.md` has the
